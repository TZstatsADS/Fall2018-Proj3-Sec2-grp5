...
```{r model_setup}
model_values <- c(100, 150, 200, 200)
model_depth <- c(3, 5, 5, 7)
...
if(run.feature.train){
  tm_feature_train <- system.time(dat_train <- feature2(train_LR_dir, train_HR_dir))
  feat_train <- dat_train$feature
  label_train <- dat_train$label
}

save(dat_train, file="../output/feature_train2.RData")
```
...
```{r loadlib}
source("../lib/XGB/train_xgboost.R")
...
source("../lib/XGB/cross_validation_xgboost.R")

if(run.cv){
  err_cv_xgboost <- array(dim=c(length(model_values), 2))
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv_xgboost[k,] <- cv.function_xgboost(feat_train, label_train, model_values[k], model_depth[k], K)
  }
  save(err_cv_xgboost, file="../output/XGB_5_5/err_cv_xgboost.RData")
}
```
...
```{r cv_vis}
if(run.cv){
  load("../output/XGB_5_5/err_cv_xgboost.RData")
...
```{r final_train}
tm_train=NA
tm_train <- system.time(fit_train_xgboost_5_5 <- train_xgboost(feat_train, label_train, par_best))
save(fit_train_xgboost_5_5, file="../output/XGB_5_5/fit_train_xgboost_5_5.RData")
```
...
```{r superresolution}
source("../lib/XGB/superResolution2.R")
test_dir <- "../data/test_set/XGB_5_5/" # This will be modified for different data sets.
test_LR_dir <- paste(test_dir, "LR/", sep="")
test_HR_dir <- paste(test_dir, "HR/", sep="")

tm_test=NA
if(run.test){
  load(file="../output/XGB_5_5/fit_train_xgboost_5_5.RData")
  tm_test <- system.time(superResolution2(test_LR_dir, test_HR_dir, fit_train_xgboost_5_5))
}
```
